...
const SCHEMA = {
  ...
  email: {
    ...
    validate: {
      ...
      noTwoDotsInARow(val) {
        if (val.includes('..')) {
          throw Error('Email cant have two dots in a row')
        }
      }
    }
  },
...
export default function(sequelize) {
  ...
  User.prototype.issueAuthToken = function(salt, authConfig) {
    return {
      accessToken: this.issueToken(TOKEN_TYPES.ACCESS_TOKEN, salt, authConfig.accessTokenLifetime),
      refreshToken: this.issueToken(TOKEN_TYPES.REFRESH_TOKEN, salt, authConfig.refreshTokenLifetime),
